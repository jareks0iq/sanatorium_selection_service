...
from adapters.repository import TagRepository, SanatoriumRepository, UserRepository, ReviewRepository
from service_layer.services import user_created, change_user_password, update_profile, create_profile, add_review
...
def route_add_review():
    ...
    add_review(user_id=data["user_id"], sanatorium_id=data["sanatorium_id"], text=data["text"],
               rating=data["rating"], created_at=datetime.now().strftime("%Y-%m-%d %H:%M"))

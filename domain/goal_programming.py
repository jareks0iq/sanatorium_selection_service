def recommend(sanatoriums: list, profile):
    ...
    sum_weights = (
        ...
        + profile.region_weight
        + profile.medical_weight
        + profile.services_weight
        + profile.conditions_weight
    )
    ...
    for s in sanatoriums:
        ...
        medical = (
            (1 - medical_good / len(profile_medical)) * w_medical if len(profile_medical) > 0 else 0
        )
        services = (
            (1 - services_good / len(profile_services)) * w_services
            if len(profile_services) > 0
            else 0
        )
        conditions = (
            (1 - conditions_good / len(profile_conditions)) * w_conditions
            if len(profile_conditions) > 0
            else 0
        )

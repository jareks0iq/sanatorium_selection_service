...
from adapters.orm import UserORM, UserProfileORM, SanatoriumORM, TagOrm, ReviewORM
from domain.model import User, UserProfile, Sanatorium, Tag, Review
...
class UserRepository:
    ...
    def get_by_login(self, login): #проверяем занят ли login и для проверки пароля
        ...
        if active_user:
            return User(id=active_user.id, name=active_user.name, login=active_user.login, password=active_user.password)
        return None
    def get_by_id(self, id): #когда пользователь залогинен и делает какие-то действия, например, смена пароля
    ...
    def create_profile(self, profile, tag_ids: list[int]):
        ...
        user_profile_orm = UserProfileORM(user_id = profile.user_id, goal = profile.goal, budget=profile.budget, region=profile.region,
                                          tags=tags_orm,
                                          budget_weight=profile.budget_weight, region_weight=profile.region_weight, medical_weight=profile.medical_weight,
                                          conditions_weight=profile.conditions_weight, services_weight=profile.services_weight)
    ...
    def get_by_user_id(self, user_id):
        ...
        if active_profile:
            return UserProfile(id=active_profile.id, user_id=active_profile.user_id, goal = active_profile.goal, budget=active_profile.budget, region=active_profile.region,
                                          tags=active_profile.tags,
                                          budget_weight=active_profile.budget_weight, region_weight=active_profile.region_weight, medical_weight=active_profile.medical_weight,
                                          conditions_weight=active_profile.conditions_weight, services_weight=active_profile.services_weight)
        return None
...
class SanatoriumRepository:
    ...
    def get_by_tags(self, tag_ids: list[int]):
        ...
        matching_ids = (
            self.session
            .query(sanatorium_tags.c.sanatorium_id)
...
class ReviewRepository():
    def __init__(self):
        self.session = SessionLocal()
    def add(self, review):
        review_orm = ReviewORM(user_id=review.user_id, sanatorium_id=review.sanatorium_id,
                           text=review.text, rating=review.rating, created_at=review.created_at)
